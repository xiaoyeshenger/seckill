package com.zy.seckill.common.redis;
...
public class RedisService {
    ...
    /**
     * 向redis中存入位置信息
     * @param key 键 , longitude 位置信息的经度 , latitude 纬度 , name 位置名称
     * @example1 “park” , 104.365986 , 30.263656 , "怡湖公园"
     * @example2 “park” , 104.351219 , 30.250031 , "凤凰湖公园"
     * @param
     * @return addedNum 添加成功的位置数量
     */
    public Long addGeoPos(String key,Double longitude, Double latitude,String name) {
    ...
    /**
     * 删除redis中存入位置信息
     * @param key 键 , positionName 位置名称
     * @example1 “park” , "怡湖公园"
     * @example2 “park” , "凤凰湖公园"
     * @param
     * @return removedNum 移除成功的位置数量
     */
    public Long removeGeoPos(String key,String positionName) {
        Long removedNum = redisTemplate.opsForGeo().remove(key, positionName);
        return removedNum;
    }

    /**
     *  从key里返回所有给定位置元素的位置（经度和纬度）。
     * @param key 键 , positionNameList 位置名称列表
     * @example1 “park” , ["怡湖公园","凤凰湖公园"]
     * @example2 “park” , ["怡湖公园"]
     * @param
     * @return  位置列表
     */
    public List<Point> getGeoPosList(String key,List<String> positionNameList) {
        List<Point> pointList = redisTemplate.opsForGeo().position(key,positionNameList);
        return pointList;
    }
    ...
    /**
     *  从key里返回所有给定位置元素的位置（经度和纬度）。
     * @param key 键 , positionNames 位置名称列表
     * @example1 “park” , ["怡湖公园","凤凰湖公园"]
     * @example2 “park” , ["怡湖公园"]
     * @param
     * @return  位置列表
     */
    public List<Point> getGeoPosList(String key,String... positionNames) {
        List<Point> pointList = redisTemplate.opsForGeo().position(key,positionNames);
        return pointList;
    }

    /**
     *  获取两个位置之间的距离
     * @param key 键 , positionName1 位置名称1, positionName2 位置名称2, distanceUnit 距离单位
     * @example1 “park” , "怡湖公园" , "凤凰湖公园" , DistanceUnit.KILOMETERS
     * @param
     * @return 距离量
     */
    public Distance getTwoPositionDistance(String key, String positionName1, String positionName2, RedisGeoCommands.DistanceUnit distanceUnit) {
        Distance distance = redisTemplate.opsForGeo()
                .distance(key,positionName1,positionName2, distanceUnit);
        return distance;
    }

    /**
     *  给定一个经纬度，返回离这个经纬度不超过distance距离的所有元素，并且返回指定key中的这些元素到给定经纬度的平均距离
     * @param key 键 , longitude 给定位置的经度, latitude 给定位置的纬度 ,distance距离量, metrics距离单位,limitCount返回的个数
     * @example1 “park” ,104.365986 , 30.263656 , 20, Metrics.KILOMETERS , 100
     * @param
     * @return 位置数据结果列表
     */
    public List<RedisGeoCommands.GeoLocation<String>> getNearPositionListByLonLat(String key,Double longitude, Double latitude,Double distanceNumber,Metrics metrics,Long limitCount){
        Circle circle = new Circle(new Point(longitude, latitude), new Distance(distanceNumber,metrics));
        ...
        List<RedisGeoCommands.GeoLocation<String>> geoLocationList = results.getContent().stream().map(GeoResult::getContent).collect(Collectors.toList());
        return geoLocationList;
    }

    /**
     *  给定一个redis中已存在的位置名称，返回离这个位置不超过distance距离的所有元素，并且返回这些元素到给定位置的平均距离
     * @param key 键 , longitude 给定位置的经度, latitude 给定位置的纬度 ,distance距离量, metrics距离单位,limitCount返回的个数
     * @example1 “park” ,"怡湖公园" , 20, Metrics.KILOMETERS , 100
     * @param
     * @return 位置数据结果列表
     */
    public List<RedisGeoCommands.GeoLocation<String>> getNearPositionListByPositionName(String key,String positionName,Double distanceNumber,Metrics metrics,Long limitCount){
        Distance distance = new Distance(distanceNumber,metrics);
        RedisGeoCommands.GeoRadiusCommandArgs args = RedisGeoCommands.GeoRadiusCommandArgs
                .newGeoRadiusArgs()
                .includeDistance()
                .includeCoordinates()
                ...
                .limit(limitCount);
        GeoResults<RedisGeoCommands.GeoLocation<String>>  results = redisTemplate.opsForGeo()
                .radius(key,positionName,distance,args);
        List<RedisGeoCommands.GeoLocation<String>> geoLocationList = results.getContent().stream().map(GeoResult::getContent).collect(Collectors.toList());
        return geoLocationList;
    }

    /**
     *  返回位置的geohash字符串，即将二维的经纬度转为以为的GeoHash字符串
     * @param key 键 , positionNameList 位置名称列表
     * @example1 “park” , ["怡湖公园","凤凰湖公园"]
     * @example2 “park” , ["怡湖公园"]
     * @param
     * @return  位置列表
     */
    public List<String> getGeoHash(String key,List<String> positionNameList){
        List<String> geoHashList = redisTemplate.opsForGeo()
                .hash(key,positionNameList);
        return geoHashList;
    }
     ...
     * @return  位置列表
     */
    public List<String> getGeoHash(String key,String... positionNames){
        List<String> geoHashList = redisTemplate.opsForGeo()
                .hash(key,positionNames);
        return geoHashList;
    }
