package com.jsxa.vapp.common.mongo;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.jsxa.vapp.common.bo.dto.WaterMeterRecordDataStatisticsVo;
import com.jsxa.vapp.common.bo.vo.ElectricityRecordDataStatisticsVo;
import com.jsxa.vapp.common.utils.ReflectUtil;
...
public abstract class MongoBaseDaoImpl<T> implements MongoBaseDao<T> {
    ...
    @Override
    public Page<T> findDistinctPageList(Query query, String uniqueKey, Integer pageIndex, Integer pageSize, Sort sort) {
        //--1.处理分页参数
        if (pageIndex <1 ){
            pageIndex = 1;
        }
        ...
        List<T> list = findDistinct(with,uniqueKey);
    ...
    @Override
    public List<T> findDistinct(Query query,String uniqueKey){
        return mongoTemplate.findDistinct(query, uniqueKey,collectionName,entityType,entityType);
    }
    ...
    public List<WaterMeterRecordDataStatisticsVo> aggregationListByWater(Aggregation aggregation) {
        AggregationResults<WaterMeterRecordDataStatisticsVo> outputType =
                mongoTemplate.aggregate(aggregation, collectionName, WaterMeterRecordDataStatisticsVo.class);
        List<WaterMeterRecordDataStatisticsVo> resultList = outputType.getMappedResults();
        return resultList;
    }

    public List<ElectricityRecordDataStatisticsVo> aggregationListByEle(Aggregation aggregation) {
        AggregationResults<ElectricityRecordDataStatisticsVo> outputType =
                mongoTemplate.aggregate(aggregation, collectionName, ElectricityRecordDataStatisticsVo.class);
        List<ElectricityRecordDataStatisticsVo> resultList = outputType.getMappedResults();
        return resultList;
    }
