...
public class ProductExcelListener extends AnalysisEventListener<ProductExcelVo> {
    ...
    private List<ProductExcelVo> productExcelVoList = new ArrayList<>();
    ...
    public ProductExcelListener(ProductService productService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    public void invoke(ProductExcelVo productExcelVo, AnalysisContext analysisContext) {
        ...
        validate(productExcelVo,currentRowNum);
    ...
    //2.参数校验
    private void validate(ProductExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(ProductExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(ProductExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<ProductExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<ProductExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        productExcelVoList.forEach(e -> {
            ProductReqDto productReqDto = VoPoConverterUtil.copyProperties(e, ProductReqDto.class);
            productService.insertOrUpdateProduct(productReqDto);
        });
    }
}