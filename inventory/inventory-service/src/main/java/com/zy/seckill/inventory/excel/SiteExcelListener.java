...
@Setter
@Component
public class SiteExcelListener extends AnalysisEventListener<SiteExcelVo> {
    ...
    private List<SiteExcelVo> siteExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public SiteExcelListener() {}

    public SiteExcelListener(SiteService siteService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(SiteExcelVo siteExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(siteExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        siteExcelVoList.add(siteExcelVo);
    }

    //2.参数校验
    private void validate(SiteExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(SiteExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(SiteExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<SiteExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<SiteExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        siteExcelVoList.forEach(e -> {
            SiteReqDto siteReqDto = VoPoConverterUtil.copyProperties(e, SiteReqDto.class);
            siteService.insertOrUpdateSite(siteReqDto);
        });
    }
}