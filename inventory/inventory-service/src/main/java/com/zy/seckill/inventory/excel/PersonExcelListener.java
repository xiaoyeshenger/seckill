...
@Setter
@Component
public class PersonExcelListener extends AnalysisEventListener<PersonExcelVo> {
    ...
    private List<PersonExcelVo> personExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public PersonExcelListener() {}

    public PersonExcelListener(PersonService personService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(PersonExcelVo personExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(personExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        personExcelVoList.add(personExcelVo);
    }

    //2.参数校验
    private void validate(PersonExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(PersonExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(PersonExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<PersonExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<PersonExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        personExcelVoList.forEach(e -> {
            PersonReqDto personReqDto = VoPoConverterUtil.copyProperties(e, PersonReqDto.class);
            personService.insertOrUpdatePerson(personReqDto);
        });
    }
}