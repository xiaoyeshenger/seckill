...
@Setter
@Component
public class ProductReleaseExcelListener extends AnalysisEventListener<ProductReleaseExcelVo> {
    ...
    private List<ProductReleaseExcelVo> productReleaseExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public ProductReleaseExcelListener() {}

    public ProductReleaseExcelListener(ProductReleaseService productReleaseService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(ProductReleaseExcelVo productReleaseExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(productReleaseExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        productReleaseExcelVoList.add(productReleaseExcelVo);
    }

    //2.参数校验
    private void validate(ProductReleaseExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(ProductReleaseExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(ProductReleaseExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<ProductReleaseExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<ProductReleaseExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        productReleaseExcelVoList.forEach(e -> {
            ProductReleaseReqDto productReleaseReqDto = VoPoConverterUtil.copyProperties(e, ProductReleaseReqDto.class);
            productReleaseService.insertOrUpdateProductRelease(productReleaseReqDto);
        });
    }
}