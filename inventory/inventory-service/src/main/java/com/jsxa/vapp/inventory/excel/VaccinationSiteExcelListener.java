...
@Setter
@Component
public class VaccinationSiteExcelListener extends AnalysisEventListener<VaccinationSiteExcelVo> {
    ...
    private List<VaccinationSiteExcelVo> vaccinationSiteExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public VaccinationSiteExcelListener() {}

    public VaccinationSiteExcelListener(VaccinationSiteService vaccinationSiteService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(VaccinationSiteExcelVo vaccinationSiteExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(vaccinationSiteExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        vaccinationSiteExcelVoList.add(vaccinationSiteExcelVo);
    }

    //2.参数校验
    private void validate(VaccinationSiteExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(VaccinationSiteExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(VaccinationSiteExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<VaccinationSiteExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<VaccinationSiteExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        vaccinationSiteExcelVoList.forEach(e -> {
            VaccinationSiteReqDto vaccinationSiteReqDto = VoPoConverterUtil.copyProperties(e, VaccinationSiteReqDto.class);
            vaccinationSiteService.insertOrUpdateVaccinationSite(vaccinationSiteReqDto);
        });
    }
}