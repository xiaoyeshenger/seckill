...
@Setter
@Component
public class VaccineExcelListener extends AnalysisEventListener<VaccineExcelVo> {
    ...
    private List<VaccineExcelVo> vaccineExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public VaccineExcelListener() {}

    public VaccineExcelListener(VaccineService vaccineService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(VaccineExcelVo vaccineExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(vaccineExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        vaccineExcelVoList.add(vaccineExcelVo);
    }

    //2.参数校验
    private void validate(VaccineExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(VaccineExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(VaccineExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<VaccineExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<VaccineExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        vaccineExcelVoList.forEach(e -> {
            VaccineReqDto vaccineReqDto = VoPoConverterUtil.copyProperties(e, VaccineReqDto.class);
            vaccineService.insertOrUpdateVaccine(vaccineReqDto);
        });
    }
}