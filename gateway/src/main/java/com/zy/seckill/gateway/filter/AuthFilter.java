package com.zy.seckill.gateway.filter;
...
import com.zy.seckill.gateway.utils.JwtTokenUtil;
import com.zy.seckill.gateway.utils.ObjUtil;
import com.zy.seckill.gateway.vo.ResultVo;
import com.zy.seckill.gateway.vo.UserVo;
...
/*        //测试请求参数
        ...
        if (!whiteList.contains(segments[1])) {
            ...
            //如何保证幂等性:
            //借助Redis单线程和INCR原子性特性，在获取token时对其进行自增操作。当客户端携带token访问执行业务代码时，继续对其进行自增，
            // 如果自增后的返回值为2，则是一个合法请求允许执行，否则认为是非法请求，直接返回。
            if(onlyOneLogin == 1){
