package com.jsxa.vapp.order.excel;
...
@Setter
@Component
public class VaccinationRecordExcelListener extends AnalysisEventListener<VaccinationRecordExcelVo> {
    ...
    private List<VaccinationRecordExcelVo> vaccinationRecordExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public VaccinationRecordExcelListener() {}

    public VaccinationRecordExcelListener(VaccinationRecordService vaccinationRecordService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(VaccinationRecordExcelVo vaccinationRecordExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(vaccinationRecordExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        vaccinationRecordExcelVoList.add(vaccinationRecordExcelVo);
    }

    //2.参数校验
    private void validate(VaccinationRecordExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(VaccinationRecordExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(VaccinationRecordExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<VaccinationRecordExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<VaccinationRecordExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        vaccinationRecordExcelVoList.forEach(e -> {
            VaccinationRecordReqDto vaccinationRecordReqDto = VoPoConverterUtil.copyProperties(e, VaccinationRecordReqDto.class);
            vaccinationRecordService.insertOrUpdateVaccinationRecord(vaccinationRecordReqDto);
        });
    }
}