...
@Setter
@Component
public class SysFileExcelListener extends AnalysisEventListener<SysFileExcelVo> {
    ...
    private List<SysFileExcelVo> sysFileExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public SysFileExcelListener() {}

    public SysFileExcelListener(SysFileService sysFileService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(SysFileExcelVo sysFileExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(sysFileExcelVo,currentRowNum);
    ...
    //2.参数校验
    private void validate(SysFileExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(SysFileExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(SysFileExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<SysFileExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<SysFileExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    private void importDataBase() {
        sysFileExcelVoList.forEach(e -> {
            SysFileReqDto sysFileReqDto = VoPoConverterUtil.copyProperties(e, SysFileReqDto.class);
            sysFileService.insertOrUpdateSysFile(sysFileReqDto);
        });
    }
}