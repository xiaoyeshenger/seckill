...
import com.jsxa.vapp.sys.service.OrganizationService;
...
@Setter
@Component
public class OrganizationExcelListener extends AnalysisEventListener<OrganizationExcelVo> {
    ...
    private List<OrganizationExcelVo> organizationExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public OrganizationExcelListener() {}

    public OrganizationExcelListener(OrganizationService organizationService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(OrganizationExcelVo organizationExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(organizationExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        organizationExcelVoList.add(organizationExcelVo);
    }

    //2.参数校验
    private void validate(OrganizationExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(OrganizationExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(OrganizationExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<OrganizationExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<OrganizationExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        organizationExcelVoList.forEach(e -> {
            OrganizationReqDto organizationReqDto = VoPoConverterUtil.copyProperties(e, OrganizationReqDto.class);
            organizationService.insertOrUpdateOrganization(organizationReqDto);
        });
    }
}