...
import com.zy.seckill.common.bo.po.*;
...
import com.zy.seckill.common.mapper.*;
import com.zy.seckill.common.messagePush.SmsClient;
...
import com.zy.seckill.sys.bo.dto.UserRegisterReqDto;
...
import static com.zy.seckill.common.utils.Sm4Util.SYS_IV;
import static com.zy.seckill.common.utils.Sm4Util.SYS_SECRET_KEY;
...
public class LoginServiceImpl implements LoginService {
    ...
    public UserVo register(UserRegisterReqDto userRegisterReqDto) {
        ...
        //1.名称是否存在
        String orgName = userRegisterReqDto.getOrgName();
        Organization organization = organizationMapper.selectByExampleOne()
                .where(OrganizationDynamicSqlSupport.name, isEqualTo(orgName))
                .build()
                .execute();
        if(!ObjUtil.isEmpty(organization)){
            throw new IllegalArgumentException("该机构名称已存在");
        }
        Long orgId = new IdWorker().nextId();
        Organization org = Organization.builder().id(orgId)
                .name(orgName)
                .code(UUIDUtil.getUUID8())
                .contactName(userRegisterReqDto.getName())
                .contactMobile(userRegisterReqDto.getMobileNumber())
                .status((byte) 1)
        ...
        //2.创建用户
        //(1)手机号是否存在
        String mobileNumber = userRegisterReqDto.getMobileNumber();
    ...
    public UserVo login(UserLoginReqDto userLoginReqDto) {
        ...
        userVo.setNickName(user.getName());
        userVo.setRegType(user.getRegType());
        userVo.setPicUrl(accessService.getAccessUrl(user.getPicUrl(), "user"));
        userVo.setUserSex(user.getUserSex());
        userVo.setMobileNumber(user.getMobileNumber());
        userVo.setRoleId(user.getRoleId());
        userVo.setRoleKey(user.getRoleKey());

        List<Map<String, Object>> orgList = organizationService.getOrgList(user.getId());
    ...
    @Override
    public UserVo freeLogin(FreeLoginReqDto userLoginReqDto) {
        //2.生成userVo对象并返回
        UserVo userVo = new UserVo();

        //1.sm4解密获取账号id
        try {
            String userId = Sm4Util.decryptCbc(SYS_SECRET_KEY,SYS_IV, userLoginReqDto.getFreeKey());

            User user = userMapper.selectByPrimaryKey(Long.valueOf(userId));

            if (ObjUtil.isEmpty(user)) {
                //保存登录日志
                AsyncJobManager.me().execute(AsyncFactory.saveLoginLog(user.getUsername(), null, Constant.LOGIN_FAIL, MessageUtil.message("user.not.exists")));
    ...
    public User queryByUsernameAndPassword(String username, String password) {
        User user = userMapper.selectOne(
                select(UserDynamicSqlSupport.id,
                        ...
                        UserDynamicSqlSupport.orgId,
                        UserDynamicSqlSupport.orgName,
